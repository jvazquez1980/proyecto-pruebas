...
import allure
import openpyxl
from functions.Inicializarchrome import Inicializarchrome
...
class Functions(Inicializar):
    ...
    ##############   -=_INICIALIZAR Chrome=-   #############################
    ##########################################################################
    def abrir_CHROME(self, URL=Inicializarchrome.URL, navegador=Inicializarchrome.NAVEGADOR):
        print("Directorio Base: " + Inicializarchrome.basedir)
        self.ventanas = {}
        print("----------------")
        print(navegador)
        print("---------------")
        ...
        if navegador == ("Chrome"):
            options = OpcionesChrome()
            options.add_argument('start-maximized')
            self.driver = webdriver.Chrome(chrome_options=options, executable_path=Inicializarchrome.basedir + "\\drivers\\chromedriver.exe")
            self.driver.maximize_window()
            self.driver.implicitly_wait(10)
            self.driver.get(URL)
            self.ventanas = {'Principal': self.driver.window_handles[0]}
    ...
    def get_text(self, entity, MyTextElement = None):
        ...
        if Get_Entity is None:
            ...
            try:
                ...
                if self.json_GetFieldBy.lower() == "xpath":
                    if MyTextElement is not None:
                        self.json_ValueToFind = selflink.json_ValueToFind.format(MyTextElement)
                        print (self.json_ValueToFind)
                    elements = self.driver.find_element_by_xpath(self.json_ValueToFind)

                if self.json_GetFieldBy.lower() == "":
                    elements = self.driver.find_element_by_partial_link_text(self.json_ValueToFind)
    ...
    def send_especific_keys(self, element, key):
        ...
        if key == 'Clicderecho':
            Functions.get_elements(self, element).send_keys(Keys.SHIFT + Keys.F10)
        time.sleep(3)
        ...
        if Get_Entity is None:
            ...
            try:
                if self.json_GetFieldBy.lower() == "id":
                    localizador = self.driver.find_element(By.ID, self.json_ValueToFind)
                    action = ActionChains(self.driver)
                    action.double_click(localizador)
                    action.perform()
                    print(u"double_click: " + locator)
                    return True
                ...
                print(u"double_click: No presente " + locator)
                Functions.tearDown(self)
                return None
    # Clic derecho

    def context_clic(self, locator):
        Get_Entity = Functions.get_entity(self, locator)
        if Get_Entity is None:
            return print("No se encontro el valor en el Json definido")
        else:
            try:
                if self.json_GetFieldBy.lower() == "id":
                    localizador = self.driver.find_element(By.ID, self.json_ValueToFind)
                    action = ActionChains(self.driver)
                    action.context_click(localizador)
                    action.perform()
                    print(u"context_clic: " + locator)
                    return True

                if self.json_GetFieldBy.lower() == "xpath":
                    localizador = self.driver.find_element(By.XPATH, self.json_ValueToFind)
                    action = ActionChains(self.driver)
                    action.context_clic(localizador)
                    action.perform()
                    print(u"context_clic: " + locator)
                    return True
                ...
                print(u"context_clic: No presente " + locator)
                Functions.tearDown(self)
                return None
    # Drag and drop

    def drag_and_drop(self, source1, target1):
        Get_Entity_From = Functions.get_entity(self, source1)
        Get_Entity_to = Functions.get_entity(self, target1)
        if Get_Entity_From is None:
            return print("No se encontro el valor en el Json definido")
        else:
            try:

                if self.json_GetFieldBy.lower() == "xpath":
                    source = self.driver.find_element(By.XPATH, self.json_ValueToFind)
                    target = self.driver.find_element(By.XPATH, self.json_ValueToFind)
                    action = ActionChains(self.driver)
                    action.drag_and_drop(source, target).perform()
                    print(u"drag_and_drop: " + source1 + target1)
                    return True

            except TimeoutException:
                print(u"drag_and_drop: No presente " + source1 + target1)
                Functions.tearDown(self)
                return None
    #slider arrastrar
        ...
        if Get_Entity is None:
            ...
            try:
                ...
                if self.json_GetFieldBy.lower() == "xpath":
                    localizador = self.driver.find_element(By.XPATH, self.json_ValueToFind)
                    action = ActionChains(self.driver)
                    action.click_and_hold(localizador)
                    action.perform()
                    print(u"click_and_hold: " + locator)
                    return True

            except TimeoutException:
                print(u"click_and_hold: No presente " + locator)
                Functions.tearDown(self)
                return None
    # slider soltar
        ...
        if Get_Entity is None:
            ...
            try:
                ...
                if self.json_GetFieldBy.lower() == "xpath":
                    localizador = self.driver.find_element(By.XPATH, self.json_ValueToFind)
                    action = ActionChains(self.driver)
                    action.release(localizador)
                    action.perform()
                    print(u"release: " + locator)
                    return True

            except TimeoutException:
                print(u"release: No presente " + locator)
                Functions.tearDown(self)
                return None
